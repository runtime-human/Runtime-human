...
pub(crate) async fn persistence_create_save_v1(
    ...
    Ok(run_blocking(move || handle.create_save(command))
        .await
        .into())
}
...
pub(crate) async fn persistence_load_save_v1(
    ...
    let handle = Arc::clone(state.inner());
    Ok(run_blocking(move || handle.load_save(query)).await.into())
}
...
pub(crate) async fn persistence_begin_month_run_v1(
    ...
    let handle = Arc::clone(state.inner());
    Ok(run_blocking(move || handle.begin_month_run(command))
        .await
        .into())
}
...
pub(crate) async fn persistence_load_month_run_v1(
    ...
    let handle = Arc::clone(state.inner());
    Ok(run_blocking(move || handle.load_month_run(query))
        .await
        .into())
}
...
pub(crate) async fn persistence_load_active_month_run_v1(
    ...
    let handle = Arc::clone(state.inner());
    Ok(run_blocking(move || handle.load_active_month_run(query))
        .await
        .into())
}
...
pub(crate) async fn persistence_store_month_run_boundary_v1(
    ...
    let handle = Arc::clone(state.inner());
    Ok(run_blocking(move || handle.store_boundary(command))
        .await
        .into())
}
...
pub(crate) async fn persistence_commit_month_run_v1(
    ...
    let handle = Arc::clone(state.inner());
    Ok(run_blocking(move || handle.commit_month_run(command))
        .await
        .into())
}
...
pub(crate) async fn persistence_create_backup_v1(
    ...
    let handle = Arc::clone(state.inner());
    Ok(run_blocking(move || handle.create_backup(command))
        .await
        .into())
}
