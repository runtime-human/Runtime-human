...
pub(crate) async fn persistence_create_save_v1(
    ...
    run_blocking(move || handle.create_save(command)).await.into()
}
...
pub(crate) async fn persistence_begin_month_run_v1(
    ...
    let handle = Arc::clone(state.inner());
    run_blocking(move || handle.begin_month_run(command)).await.into()
}
...
pub(crate) async fn persistence_load_month_run_v1(
    ...
    let handle = Arc::clone(state.inner());
    run_blocking(move || handle.load_month_run(query)).await.into()
}
...
pub(crate) async fn persistence_load_active_month_run_v1(
    ...
    let handle = Arc::clone(state.inner());
    run_blocking(move || handle.load_active_month_run(query)).await.into()
}
...
pub(crate) async fn persistence_store_month_run_boundary_v1(
    ...
    let handle = Arc::clone(state.inner());
    run_blocking(move || handle.store_boundary(command)).await.into()
}
