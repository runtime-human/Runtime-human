...
pub(crate) fn configure_webview_data_directory() -> Result<(), io::Error> {
    let data_directory = required_app_data_directory_override()?;
    let webview_directory = webview_data_directory(&data_directory);
    std::fs::create_dir_all(&webview_directory)?;

    // SAFETY: this runs at process entry, before Tauri or any application worker
    // thread exists. WebView2 reads the environment while creating its process.
    unsafe {
...
fn webview_data_directory(data_directory: &std::path::Path) -> PathBuf {
...
mod tests {
    ...
    use super::{parse_data_directory_argument, webview_data_directory};
    ...
    #[test]
    fn keeps_webview_user_data_inside_the_isolated_root() {
        let root = absolute_test_directory();
        assert_eq!(webview_data_directory(&root), root.join("webview"));
    }
