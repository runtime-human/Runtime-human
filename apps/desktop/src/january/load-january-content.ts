...
export async function loadJanuaryContentRegistry(
  ...
  const manifest = await performance.measure("content.load_manifest", async () =>
  ...
  const chunks = await Promise.all(
    requiredChunkIds.map((chunkId) =>
      performance.measure("content.load_chunk", async () =>
  ...
  return performance.measure("content.publish_registry", async () =>
