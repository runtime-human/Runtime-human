...
describe("CareerOverviewScreen", () => {
  ...
  it("shows the persisted active stage, revision and progress", () => {
    ...
    expect(screen.getByText("Сохранённая версия 4")).toBeInTheDocument();
  ...
  it("shows completed quality scores with their real metric maxima", () => {
    ...
    expect(screen.getByText("Версия сохранения 1")).toBeInTheDocument();
  ...
  it("keeps internal runtime vocabulary out of player-facing copy", () => {
    ...
    expect(screen.getByText("Несовместимое сохранение")).toBeInTheDocument();
    expect(container).not.toHaveTextContent(/Persisted|MonthRun|checkpoint|коммит/iu);
  });
