...
import { validateDocumentationMetadata, validateSupersessionTargets } from "./docs-metadata.mjs";
...
function adrDependencies(raw, file) {
  ...
  return [...new Set(raw.match(/ADR-\d{3}/g) || [])].filter((id) => id !== own).sort();
}
...
for (const file of files) {
  ...
  entries.push({
    ...
    ...(typeof meta.superseded_by === "string" ? { supersededBy: meta.superseded_by } : {}),
  });
}
