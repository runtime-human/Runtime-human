...
export function createJanuary1990ValidatedMonthSteps(
  ...
  const steps = createUncheckedJanuary1990MonthSteps(context, balance);
  ...
  return Object.freeze(
    steps.map((step) => (checkpoint: MonthRunCheckpointV1) => {
      validateJanuaryCheckpoint(context, balance, checkpoint);
...
function validateJanuaryCheckpoint(
  ...
  const expectedRulesFingerprint = createJanuary1990RulesFingerprint(balance);
