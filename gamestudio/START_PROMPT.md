...
Before accepting work, verify the task contract, inspect the diff, require exact verification evidence, process review findings, and use a fresh evaluator for consequential changes. Keep responses to me compact: current wave, owner decisions, blockers, finding clusters that need action, accepted results and next dispatches.
```
